...
function resolveRound(room, players, currentBet, minRaise, fallbackTurnUid) {
  ...
  if (contenders.length === 1) {
    ...
    const finalPlayers = applyRefunds(players, refunds);
    return {
      ...room,
      ...
      pots,
      lastHandResult: `${finalPlayers[contenders[0].uid].name} wins, everyone else folded.`,
...
export function advanceStreet(room) {
  ...
  if (idx === streets.length - 1) {
    ...
    const { pots, refunds } = computeSidePots(room.players);
    return {
      ...room,
      players: applyRefunds(room.players, refunds),
      status: "showdown",
      turnUid: null,
      pots,
    };
...
export function computeSidePots(players) {
  ...
  //
  // Edge case: if a layer's contributors are ALL folded (or busted), nobody
  // is left to contest that slice, it was never really at risk. That happens
  // when the biggest bettor of the hand ends up folding after everyone who
  // could have called them was already all-in for less or already folded.
  // That slice is an uncalled bet: it goes straight back to whoever put it
  // in, never into a pot with no possible winner.
  ...
  while (contributors.some((c) => c.remaining > 0)) {
    const layerContributors = contributors.filter((c) => c.remaining > 0);
    const layer = Math.min(...layerContributors.map((c) => c.remaining));
    const eligible = [];
    ...
    if (eligible.length === 0) {
      for (const c of layerContributors) refunds[c.uid] = (refunds[c.uid] || 0) + layer;
    } else {
      pots.push({ amount: layer * layerContributors.length, eligibleUids: eligible });
    }
  }
  return { pots, refunds };
}

function applyRefunds(players, refunds) {
  if (Object.keys(refunds).length === 0) return players;
  const next = { ...players };
  for (const [uid, amt] of Object.entries(refunds)) {
    next[uid] = {
      ...next[uid],
      stack: next[uid].stack + amt,
      handContributed: next[uid].handContributed - amt,
    };
  }
  return next;
}
...
export function awardPots(room, winnerSelections) {
  ...
  for (const p of Object.values(players)) {
    ...
    // The hand is fully settled now, nothing is "still in the pot" anymore.
