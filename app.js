...
function renderAdminPanel() {
  const seatedCount = Poker.activeSeatOrder(room).length;
  const needsAttention = room.status === "hand_complete" && seatedCount < 2;
  const rows = Object.entries(room.players)
    .filter(([, p]) => !p.sittingOut)
    .sort((a, b) => a[1].seat - b[1].seat)
    .map(
      ([uid, p]) => `<div class="chip-adjust-row">
        <span>${escapeHtml(p.name)}${p.status === "busted" ? ' <span class="tag-allin">busted</span>' : ""}</span>
        <button class="btn-small" data-adjust="-50" data-uid="${uid}">-50</button>
        <button class="btn-small" data-adjust="50" data-uid="${uid}">+50</button>
      </div>`
    )
    .join("");
  return `<details class="admin-panel"${needsAttention ? " open" : ""}>
    <summary>Admin controls</summary>
    ${
      needsAttention
        ? `<div class="pot-explainer">Give a busted player chips below to bring them back in.</div>`
        : ""
    }
    ${room.status === "playing" && room.turnUid ? `<button id="btn-force-end" class="btn btn-secondary">Force end betting round</button>` : ""}
    <div class="chip-adjust-list">${rows}</div>
  </details>`;
}
  ...
  const potsHTML = room.pots
    .map((pot, i) => {
      const eligible = pot.eligibleUids
        .filter((u) => room.players[u])
        .map((u) => ({ uid: u, ...room.players[u] }));
...
function wireShowdownEvents() {
  el("btn-award")?.addEventListener("click", () => {
    ...
    const allSelectedUids = Object.values(selections).flat();
    const bogus = allSelectedUids.filter((uid) => !room.players[uid]);
    if (bogus.length > 0) {
      // Should never happen, but this is exactly the class of bug that once
      // let chips vanish silently, so fail loudly instead of awarding nothing.
      return showToast("Something went wrong reading the winner selection. Please reload and try again.");
    }
