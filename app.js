...
function renderGame(isAdmin) {
  ...
  if (room.status === "hand_complete") {
    actionHTML = `<div class="hand-result">${escapeHtml(room.lastHandResult || "")}</div>
      ${isAdmin ? `<button id="btn-next-hand" class="btn btn-primary">Start next hand</button>` : `<div class="wait-msg">Waiting for admin to start the next hand.</div>`}`;
...
function renderShowdown(isAdmin) {
  const potsHTML = room.pots
    .map((pot, i) => {
      const eligible = pot.eligibleUids.map((u) => room.players[u]);
    ...
    .join("");
  return `
    <div class="table-header"><div class="street-label">Showdown</div></div>
...
function wireShowdownEvents() {
  el("btn-award")?.addEventListener("click", () => {
    ...
    if (Object.keys(selections).length === 0) return showToast("Pick at least one winner.");
...
el("btn-summary").addEventListener("click", () => {
  ...
  const rows = Object.values(room.players)
    ...
    .join("");
  el("summary-rows").innerHTML = rows;
});
